...
import { ShoppingBasket, Package, Plus, Minus, Leaf, Star, MapPin, Check, Loader2 } from 'lucide-react'
import { PlanType, PackageType, DBFarm, DBProduct, DBPackage, SUBSCRIPTION_PLANS } from '../types'

interface PackageSelectionProps {
  selectedPlan: PlanType
}

export default function PackageSelection({ selectedPlan }: PackageSelectionProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const farmsResponse = await fetch('/api/subscription/farms')
        if (!farmsResponse.ok) {
          const farmsError = await farmsResponse.text()
          throw new Error(`Failed to fetch farms: ${farmsResponse.status} ${farmsResponse.statusText}. ${farmsError}`)
        }

        
        const packagesResponse = await fetch('/api/subscription/packages')
        if (!packagesResponse.ok) {
          const packagesError = await packagesResponse.text()
          throw new Error(`Failed to fetch packages: ${packagesResponse.status} ${packagesResponse.statusText}. ${packagesError}`)
        }
  ...
  const calculateItemPrice = (basePrice: number) => {
    return basePrice * (1 - SUBSCRIPTION_PLANS[selectedPlan].productDiscount / 100)
  }

  const calculateCustomTotal = () => {
    return Object.entries(customItems).reduce((total, [itemId, quantity]) => {
      const farm = farms.find(f => f.products.some(p => p.id === Number(itemId)))
  ...
  const calculateTotalSavings = () => {
    const originalTotal = Object.entries(customItems).reduce((total, [itemId, quantity]) => {
      const farm = farms.find(f => f.products.some(p => p.id === Number(itemId)))
      const item = farm?.products.find(p => p.id === Number(itemId))
  ...
  const handleProceedToCheckout = () => {
    ...
    } else if (packageType === 'custom' && Object.keys(customItems).length > 0) {
    ...
    <div className="max-w-7xl mx-auto">
      ...
      {packageType === 'premade' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {packages.filter(pkg => pkg.plan_type === selectedPlan).map((pkg: DBPackage) => (
            <div
              key={pkg.package_id}
            ...
            >
              ...
              <div className={`p-6 ${selectedPackage === pkg.package_id ? 'bg-green-50' : ''}`}>
                ...
                <div className="flex flex-col mb-4">
                  <div className="flex items-baseline">
                    ...
                    <span className="text-sm text-gray-600 ml-2">with your {selectedPlan} plan</span>
                  </div>
                ...
                >
                  {selectedPackage === pkg.package_id ? 'Selected' : 'Select Package'}
                </button>
              </div>
                  ...
                  <div className="p-6 bg-white">
                    <div className="space-y-6">
                      ...
                      <div className="bg-green-50 rounded-lg p-4">
                        <h4 className="font-medium text-green-800 mb-3">Package Summary</h4>
                        <ul className="space-y-2 text-sm text-green-700">
                          ...
                          <li className="flex items-center">
                            <Check className="w-4 h-4 mr-2" />
                            Retail value: £{Number(pkg.retail_value).toFixed(2)}
                          </li>
                          <li className="flex items-center">
                            <Check className="w-4 h-4 mr-2" />
                            From: {pkg.farm_name}
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          <div className="flex items-start">
                            ...
                            <div>
                              <p className="text-sm text-gray-600">Proceed to checkout to confirm your delivery details</p>
                            </div>
                          </div>
                          <div className="flex items-start">
                            <div className="flex-shrink-0 w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mr-3">
                            ...
                            <div>
                              <p className="text-sm text-gray-600">Choose your preferred delivery day</p>
                            </div>
                          </div>
                          <div className="flex items-start">
                            <div className="flex-shrink-0 w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mr-3">
                            ...
                            <div>
                              <p className="text-sm text-gray-600">Complete your subscription setup</p>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <button
                        onClick={() => {
                          ...
                          const selectedPkg = packages.find(p => p.package_id === pkg.package_id);
                          localStorage.setItem('selectedPackage', JSON.stringify({
                            type: 'premade',
                            plan: selectedPlan,
                            package: selectedPkg
              ...
              {farms.map(farm => (
                <div
                  ...
                  className={`cursor-pointer p-4 rounded-lg border-2 ${
                    selectedFarm === farm.farm_id ? 'border-green-500 bg-green-50' : 'border-gray-200'
                  }`}
                >
                  <div>
                      ...
                      <div key={product.id} className="p-4 border rounded-lg bg-white">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="w-8 text-center">{customItems[product.id] || 0}</span>
                            <button
                              onClick={() => handleQuantityChange(product.id, 1)}
                              className="p-1 rounded-full bg-gray-100 hover:bg-gray-200"
                            >
                        ...
                        {customItems[product.id] > 0 && (
                          <p className="text-xs text-gray-500 mt-1">
                            Total: {formatTotalQuantity(product.unit, customItems[product.id])}
                          </p>
                        )}
                      </div>
                    )
                    ...
                    <div className="p-6">
                      <div className="space-y-4">
                        ...
                        <div className="bg-gray-50 rounded-lg p-4">
                          <p className="text-sm text-gray-600 mb-3">Selected items:</p>
                          <div className="space-y-2">
                            {Object.entries(customItems).map(([itemId, quantity]) => {
                              const farm = farms.find(f => f.products.some(p => p.id === Number(itemId)))
                              const item = farm?.products.find(p => p.id === Number(itemId))
                              if (!item) return null

                              const originalPrice = item.price * quantity
                              const discountedPrice = calculateItemPrice(item.price) * quantity

                              return (
                                <div key={itemId} className="flex justify-between text-sm">
                                  <span className="flex items-center">
                                    <Check className="w-4 h-4 text-green-500 mr-2" />
                                    {item.name} ({formatTotalQuantity(item.unit, quantity)})
                                  </span>
                                  <span>
                        ...
                        <div className="border-t border-gray-100 pt-4">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="font-medium">£{calculateCustomTotal().toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between items-center text-green-600">
                            <span>Total Savings</span>
                            <span className="font-medium">£{calculateTotalSavings().toFixed(2)}</span>
                          </div>
                        </div>
                        ...
                        <div className="border-t border-gray-100 pt-4 mt-6">
                          ...
                          <div className="space-y-3">
                            <div className="flex items-start">
                              ...
                              <div>
                                <p className="text-sm text-gray-600">Review your custom selection</p>
                              </div>
                            </div>
                            <div className="flex items-start">
                              <div className="flex-shrink-0 w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mr-3">
                              ...
                              <div>
                                <p className="text-sm text-gray-600">Choose your delivery preferences</p>
                              </div>
                            </div>
                            <div className="flex items-start">
                              <div className="flex-shrink-0 w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mr-3">
